...
import { MARK_LECTURE_COMPLETED_MUTATION } from "@/lib/graphql/progress";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await authFetchGraphQL(MARK_LECTURE_COMPLETED_MUTATION, {
      courseId,
      lectureId,
    });

    // response is { markLectureCompleted: {...} }
    return NextResponse.json({
      data: response,
    });
  } catch (error) {
